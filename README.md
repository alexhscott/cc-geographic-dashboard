...
- Use Optimize Routes to generate a visit schedule for any region — it clusters clients into stops within a ~3hr drive and gives you an ordered route starting from the regional training center
...
Hit **Optimize Routes**, pick a region. It finds all the counties with clients, groups the ones that are close together into stops, and orders the whole trip starting from the training center. For Northwest it also locks in the Puget Sound Field Office as a required stop.

Each stop shows how many clients are within range and which counties are covered. Click a stop to zoom the map to it. The dashed circles are the ~3hr drive radius.

---

## Regions

Follows Canine Companions' official six regions. California and Nevada are split by county since they each fall into two regions.

| Region | States |
|---|---|
| Northeast | ME, NH, VT, MA, RI, CT, NY, NJ, PA, DE, MD, DC, VA, WV |
...
Southern CA counties (LA, San Diego, Orange, Riverside, San Bernardino, Ventura, Santa Barbara, San Luis Obispo, Imperial, Kern) go to Southwest. Everything else in CA goes to Northwest. Clark County (Las Vegas) is the only NV county in Southwest.
