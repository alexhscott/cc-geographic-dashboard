  ...
  if (!info || info.count === 0 || regionMismatch) {
    ...
    info.clients.forEach(client => {
      ...
      if (phone || email) {
        ...
        if (email) actionsHtml += `<button class="client-action-btn email-open-btn" data-email="${email.replace(/"/g,'&quot;')}" title="Open in email client">✉️ Email</button>`;
        actionsHtml += '</div>';
      }
  ...
  Object.entries(byCountry).sort(([a],[b]) => a.localeCompare(b)).forEach(([country, clients]) => {
    ...
    clients.forEach(client => {
      ...
      if (iPhone || iEmail) {
        ...
        if (iEmail) intlActions += `<button class="client-action-btn email-open-btn" data-email="${iEmail.replace(/"/g,'&quot;')}" title="Open in email client">✉️ Email</button>`;
        intlActions += '</div>';
