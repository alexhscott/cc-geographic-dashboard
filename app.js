...
function getStateRegionIdx(abbr) {
  ...
  if (abbr === 'CA') return [4, 5]; // Southwest (south) + Northwest (north)
  if (abbr === 'NV') return [4, 5]; // Southwest (south) + Northwest (north)
  return [STATE_REGION_DEFAULT[abbr] ?? 0];
}

// Returns true if the state belongs to the given region (handles CA/NV split)
...
function fixSplitStateRegions() {
  ...
  Object.values(clientData).forEach(info => {
    if (info.state !== 'CA' && info.state !== 'NV') return;
