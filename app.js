...
function getStateRegionIdx(abbr) {
  // CA and NV are split between two regions — return both possible indices
...
// Does a state belong to the given region? Handles split states.
...
function setState(abbr, name) {
  ...
  if (activeState === abbr) {
    ...
    // Split states (CA/NV): keep current active region if it's one of their regions,
    // otherwise use the first (primary) region for that state
    if (rIdxs.length === 1) {
      activeRegion = String(rIdxs[0]);
    } else {
      // Only override if current activeRegion doesn't include this state
...
function buildReverseLookup() {
  ...
  // Re-assign regionIdx for any split-state (CA/NV) clients that were
  // processed before the lookup was ready and got the wrong default.
