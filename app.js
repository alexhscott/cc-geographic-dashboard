...
// Mobile sidebar — open/close on small screens
...
function onMobileFilterSelect() {
  if (window.innerWidth <= 767) closeSidebar();
}

// Region setup — colors and state mappings
...
// CA and NV span two regions. The split is handled per-county via FIPS.
// If a CSV has a Region column, that takes priority over the auto-assignment.
...
// App state — tracks active filters, uploaded data, map elements
...
const sfBaseUrl  = 'https://canine.lightning.force.com';

// Map loading — fetch county/state boundaries from CDN
...
function renderMap(topo) {
  ...
  // Build the county name → FIPS lookup
  buildReverseLookup();
  // Set up the projection for org location markers
  initOrgProjection();
}

// Zoom — animate the SVG viewBox when drilling into a region, state, or county
...
// County selection — click a county to zoom in and pull up client info
...
function openCountyPanel(countyName, stateAbbr, fips) {
  ...
  if (!info || info.count === 0 || regionMismatch) {
    ...
    info.clients.forEach(client => {
      ...
      const sfLink    = tracker && sfBaseUrl ? `${sfBaseUrl}/lightning/r/ERIDT1__Client_Tracker__c/${tracker}/view` : '';

      // Standard display fields (dog name, category)
      let fieldsHtml = '';
      DISPLAY_FIELDS.forEach(({ header, label, icon }) => {
        ...
        fieldsHtml += `
          <div class="client-field client-field-row">
            ...
            <span class="client-field-label">${label || header}:</span>
            <span class="client-field-value">${val}</span>
          </div>`;
      });

      // Out of compliance warning
      const status = f['Current Placement: Status'] || f['Status'] || '';
      if (status.toLowerCase().includes('out of compliance')) {
        fieldsHtml += `<div style="color:#dc2626;font-weight:700;font-size:0.72rem;margin-top:4px">⚠️ Out of Compliance</div>`;
      }

      // Action buttons for phone and email
      let actionsHtml = '';
      const phone = f['Phone'] || '';
      const email = f['Email'] || '';
      if (phone || email) {
        actionsHtml = '<div class="client-actions">';
        if (phone) actionsHtml += `<button class="client-action-btn phone-copy-btn" data-phone="${phone.replace(/"/g,'&quot;')}" title="Copy phone number">📞 Copy Phone</button>`;
        if (email) actionsHtml += `<button class="client-action-btn email-open-btn" data-email="${email.replace(/"/g,'&quot;')}" title="Copy email address">✉️ Copy Email</button>`;
        actionsHtml += '</div>';
      }
      ...
      row.innerHTML = `
        ...
        <div class="client-details">
          <div class="client-name">${sfLink ? `<a href="${sfLink}" target="_blank" style="color:inherit;text-decoration:none;cursor:pointer">${fullName}</a>` : fullName}</div>
...
// CSV upload — read the file and kick off the column mapping modal
document.getElementById('csv-upload').addEventListener('change', function(e) {
  const file = e.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  const fileName = e.target.files[0].name;
  reader.onload = ev => parseCSV(ev.target.result, fileName);
...
// Column mapping modal — let the user tell us which CSV column is which
// Auto-detected column indices — set in openModal, used in confirmMapping
let _autoCounty  = null, _autoState  = null, _autoFirst = null;
let _autoLast    = null, _autoTracker = null, _autoRegion = null;

function openModal() {
  // Silently auto-detect all key columns from headers
  csvHeaders.forEach((h, i) => {
    const hl = h.toLowerCase().trim();
    if (hl.includes('county'))                                              _autoCounty  = i;
    if (hl === 'state' || hl === 'mailing state/province' || hl === 'mailing state' || hl === 'billing state/province') _autoState = i;
    if (hl === 'first name' || hl === 'firstname' || hl === 'first')        _autoFirst   = i;
    if (hl === 'last name'  || hl === 'lastname'  || hl === 'last' || hl === 'surname') _autoLast = i;
    if (hl === 'current placement: record id' || hl === 'record id' || hl.includes('record id') || hl.includes('client id')) _autoTracker = i;
    if (hl.includes('region'))                                               _autoRegion  = i;
  });

  // Show filename and detected column summary
  const filenameEl = document.getElementById('modal-filename');
  if (filenameEl) filenameEl.textContent = csvFileName || 'Unknown file';

  const previewEl = document.getElementById('modal-col-preview');
  if (previewEl) {
    const detected = [
      _autoCounty  !== null ? `✓ County → ${csvHeaders[_autoCounty]}`   : '⚠ County not found',
      _autoState   !== null ? `✓ State → ${csvHeaders[_autoState]}`     : '⚠ State not found',
      _autoFirst   !== null ? `✓ First Name → ${csvHeaders[_autoFirst]}` : null,
      _autoLast    !== null ? `✓ Last Name → ${csvHeaders[_autoLast]}`   : null,
      _autoTracker !== null ? `✓ Record ID → ${csvHeaders[_autoTracker]}`: null,
    ].filter(Boolean);
    previewEl.innerHTML = detected.map(d => `<div style="color:${d.startsWith('⚠') ? '#dc2626' : '#22c55e'}">${d}</div>`).join('');
  }
...
function confirmMapping() {
  if (_autoCounty === null || _autoState === null) {
    alert('Could not detect County or State columns in this CSV. Please check the file and try again.');
    closeModal();
    return;
  }
...
// Data processing — parse the CSV rows and build the clientData lookup
// Track which CSV columns hold the key fields
...
// Fields to show in the client detail card, in display order
const DISPLAY_FIELDS = [
  { header: 'Current Placement: Dog Name',         label: 'Dog Name',        icon: '🐾' },
  { header: 'Current Placement: Outcome Category', label: 'Category',        icon: '🏷️' },
];

// Action fields — shown as buttons, value never displayed as plain text
const ACTION_FIELDS = [
...
// Map coloring — fill each county based on client count and active filters
function colorMap() {
  Object.entries(countyPaths).forEach(([fips, el]) => {
    ...
    // For split states (CA/NV): also require region match so only the correct
    // portion of the state shows (e.g. NW California only when Northwest is active)
    const stateMatch  = activeState === 'all' || (
      stateAbbr === activeState && (activeRegion === 'all' || regionIdx === +activeRegion)
    );
...
// Density scale — controls the color ramp based on slider settings
let densityMax   = 50;
let densityBands = 10;
...
// Filter tabs — switch between Region and State views in the sidebar
...
// Region filter buttons — build and update the region list in the sidebar
...
// State filter list — grouped by region, with live search
...
function getStateRegionIdx(abbr) {
  // CA and NV each span two regions, so return both
  if (abbr === 'CA') return [4, 5]; // Southwest + Northwest
  if (abbr === 'NV') return [4, 5]; // Southwest + Northwest
  // PA spans two regions — western counties are North Central, eastern are Northeast
  if (abbr === 'PA') return [0, 1]; // Northeast + North Central
  return [STATE_REGION_DEFAULT[abbr] ?? 0];
}

// Returns true if the state belongs to the given region (handles CA/NV/PA split)
function stateInRegion(abbr, regionIdx) {
  return getStateRegionIdx(abbr).includes(+regionIdx);
}
...
function updateStateList(query) {
  ...
  ALL_STATES.forEach(s => {
    ...
    const rIdxs = getStateRegionIdx(s.abbr);
    rIdxs.forEach(rIdx => groups[rIdx].push(s));
  });
  ...
  groups.forEach((states, rIdx) => {
    ...
    states.forEach(s => {
      // Count clients in this state, filtered by the active region (rIdx from group loop)
      const count = Object.values(clientData)
        .filter(d => d.state === s.abbr && d.regionIdx === rIdx)
...
function setState(abbr, name) {
  ...
  if (activeState === abbr) {
    ...
    activeState = abbr;
    const rIdxs = getStateRegionIdx(abbr);
    // For CA/NV — keep the current region if it's valid for this state
    if (rIdxs.length === 1) {
      activeRegion = String(rIdxs[0]);
    } else {
      // If the current region filter already covers this state, keep it
      if (!rIdxs.includes(+activeRegion)) {
        activeRegion = String(rIdxs[0]);
      }
    }
    zoomToSelection();
...
// Filter chips — show active region/state filters on top of the map
...
// International clients — separate panel for non-US addresses
    ...
    clients.forEach(client => {
      ...
      DISPLAY_FIELDS.forEach(({ header, label, icon }) => {
        ...
        fieldsHtml += `<div class="intl-field">
          <span class="intl-field-label">${icon} ${label || header}:</span>
          <span class="intl-field-value">${val}</span>
        </div>`;
      });

      // Out of compliance warning
      const intlStatus = f['Current Placement: Status'] || f['Status'] || '';
      if (intlStatus.toLowerCase().includes('out of compliance')) {
        fieldsHtml += `<div style="color:#dc2626;font-weight:700;font-size:0.72rem;margin-top:4px;grid-column:1/-1">⚠️ Out of Compliance</div>`;
      }

      // Action buttons for phone/email in intl panel
      const iPhone = f['Phone'] || '';
      const iEmail = f['Email'] || '';
      let intlActions = '';
      if (iPhone || iEmail) {
        intlActions = '<div class="client-actions" style="margin-top:6px">';
        if (iPhone) intlActions += `<button class="client-action-btn phone-copy-btn" data-phone="${iPhone.replace(/"/g,'&quot;')}" title="Copy phone">📞 Copy Phone</button>`;
        if (iEmail) intlActions += `<button class="client-action-btn email-open-btn" data-email="${iEmail.replace(/"/g,'&quot;')}" title="Copy email address">✉️ Copy Email</button>`;
        intlActions += '</div>';
        fieldsHtml += intlActions;
      }
      ...
      row.innerHTML = `
        <div class="intl-client-name">
          ${sfLink ? `<a href="${sfLink}" target="_blank" style="color:inherit;text-decoration:none">${fullName}</a>` : fullName}
...
function updateContextLegend() {
  ...
  if (activeState !== 'all') {
    ...
    const rIdxArr = getStateRegionIdx(activeState);
    const rIdx = activeRegion !== 'all' ? +activeRegion : rIdxArr[0];
    const color = REGION_COLORS[rIdx];
  ...
  if (activeRegion !== 'all') {
    ...
    const regionStates = ALL_STATES
      .filter(s => stateInRegion(s.abbr, rIdx))
      .map(s => {
        const count = Object.values(clientData)
          .filter(d => d.state === s.abbr && d.regionIdx === rIdx)
          .reduce((sum, d) => sum + d.count, 0);
...
// Header stats — update the totals shown at the top
...
// Utility functions — FIPS lookup, state normalization, key building
...
// CA splits at the county level: southern counties go to Southwest, northern to Northwest
...
// PA is split: western counties go to North Central, eastern counties go to Northeast
const NC_PA_FIPS = new Set([
  '42003', '42005', '42007', '42009', '42013', '42019', '42021', '42023', '42027', '42031', '42033', '42039', '42047', '42049', '42051', '42053', '42059', '42061', '42063', '42065', '42073', '42083', '42085', '42111', '42121', '42123', '42125', '42129'
]);
...
function getRegionByFips(fipsStr) {
  ...
  if (stateFips === '42') return NC_PA_FIPS.has(fips) ? 1 : 0; // 1=North Central, 0=Northeast
...
function getDefaultRegion(stateAbbr, county) {
  // CA, NV, and PA are split at the county level — look up via FIPS
  if (stateAbbr === 'CA' || stateAbbr === 'NV' || stateAbbr === 'PA') {
    ...
    if (stateAbbr === 'PA') return 0; // fallback to Northeast
    return 5; // fallback to Northwest for CA/NV
  }
  return STATE_REGION_DEFAULT[stateAbbr] ?? 0;
}
...
function buildReverseLookup() {
  ...
  // Fix CA/NV region assignments if the CSV was uploaded before the map finished loading
  fixSplitStateRegions();
}

function fixSplitStateRegions() {
  let changed = false;
  Object.values(clientData).forEach(info => {
    if (info.state !== 'CA' && info.state !== 'NV' && info.state !== 'PA') return;
    const correctRegion = getDefaultRegion(info.state, info.county);
    if (info.regionIdx !== correctRegion) {
      info.regionIdx = correctRegion;
      changed = true;
    }
  });
  if (changed) colorMap();
...
// CC org locations — training centers, field office, and volunteer chapters
...
// Route optimizer — clusters client counties and builds an efficient visit order
...
let RADIUS_MILES  = 150;
// SVG pixel radius scales with miles — calibrated so 150mi = 60px in Albers USA projection
let RADIUS_SVG_PX = Math.round(RADIUS_MILES * 0.4);
...
// Update the radius display and hint text when the slider moves
...
function launchRouteForRegion(regionIdx) {
  ...
  // Read the radius slider value and update the globals before computing
  const sliderVal = document.getElementById('route-radius-slider');
  if (sliderVal) {
    RADIUS_MILES  = +sliderVal.value;
    RADIUS_SVG_PX = Math.round(RADIUS_MILES * 0.4);
  }
...
// Groups client counties into stops, then orders them starting from home base
function computeRoute(regionIdx, home) {
  ...
  // Seed fixed stops (campus/field office) first, then cluster remaining counties
  ...
  // Refinement — reassign counties to nearer clusters if one exists (only move counties between dynamic clusters, not away from fixed)
  ...
  // Order the stops — always go to the closest unvisited one next
  const ordered = [];
...
// Draw the route on the map — circles, dashed lines, numbered markers
function renderRouteOnMap(home) {
  ...
  // Connect the dots — home to stop 1, stop 1 to stop 2, etc.
  ...
  routeStops.forEach(stop => {
    ...
    if (stop.isFixed) {
      // Fixed locations get square markers — blue for campus, yellow for field office
      ...
      // Regular meeting stops — purple circles
      const circle = document.createElementNS('http://www.w3.org/2000/svg','circle');
      circle.setAttribute('cx', stop.cx); circle.setAttribute('cy', stop.cy);
...
// Route results panel — list each stop with drive time and client count
...
// Global click handler for phone copy and email buttons
// Using event delegation so it works on dynamically created elements
document.addEventListener('click', function(e) {
  const phoneBtn = e.target.closest('.phone-copy-btn');
  if (phoneBtn) {
    const phone = phoneBtn.dataset.phone;
    if (!phone) return;
    navigator.clipboard.writeText(phone).then(() => {
      phoneBtn.textContent = '✓ Copied!';
      phoneBtn.style.color = '#22c55e';
      phoneBtn.style.borderColor = '#22c55e';
      setTimeout(() => {
        phoneBtn.innerHTML = '📞 Copy Phone';
      ...
      phoneBtn.textContent = '✓ Copied!';
      setTimeout(() => { phoneBtn.innerHTML = '📞 Copy Phone'; }, 1800);
    });
    return;
  }

  const emailBtn = e.target.closest('.email-open-btn');
  if (emailBtn) {
    const email = emailBtn.dataset.email;
    if (!email) return;
    navigator.clipboard.writeText(email).then(() => {
      emailBtn.textContent = '✓ Copied!';
      emailBtn.style.color = '#22c55e';
      emailBtn.style.borderColor = '#22c55e';
      setTimeout(() => {
        emailBtn.innerHTML = '✉️ Copy Email';
      ...
      document.body.removeChild(ta);
      emailBtn.textContent = '✓ Copied!';
      setTimeout(() => { emailBtn.innerHTML = '✉️ Copy Email'; }, 1800);
    });
  }
});
document.addEventListener('DOMContentLoaded', () => {
  buildDensityLegend();
  // Build the region buttons in the route optimizer modal
